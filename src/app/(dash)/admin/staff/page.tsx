...
import { addStaff, updateStaff, toggleStaff, resetStaffPassword, unlockStaff } from "./actions";
import { emailLockedFor } from "@/lib/login-guard";
...
export default async function StaffAdmin({ searchParams }: {
  ...
  /* Who is currently locked out by wrong passwords, and for how much longer.
     Read once for the whole list rather than per row, and by email because
     that is what the counter is keyed on. */
  const lockedMinutes = new Map<number, number>(
    rows.map((u) => [u.id, emailLockedFor(u.email)]).filter(([, m]) => m > 0) as [number, number][],
  );
          ...
          <section key={u.id} className={`border border-[--line] ${u.isActive ? "bg-white/50" : "bg-clay/5"}`}>
            <details>
              <summary className="px-5 py-4 cursor-pointer flex flex-wrap items-center gap-4 hover:bg-white">
                ...
                {/* Visible without opening the row, because the question this
                    screen gets asked in a hurry is "why can't Priya get in". */}
                {lockedMinutes.get(u.id) ? (
                  <span className="accent text-[0.6rem] text-clay">
                    Locked out &middot; {lockedMinutes.get(u.id)} min
                  </span>
                ) : null}
              </summary>
              ...
              <div className="px-5 pb-5 pt-1 bg-white">
                ...
                <div className="flex flex-wrap gap-2 mt-5 pt-4 border-t border-[--line]">
                  {lockedMinutes.get(u.id) ? (
                    <form action={unlockStaff}>
                      <input type="hidden" name="id" value={u.id} />
                      <button className="text-xs border border-clay/40 text-clay px-3 py-1.5 hover:bg-clay/5">
                        Let them try again
                      </button>
                    </form>
                  ) : null}
